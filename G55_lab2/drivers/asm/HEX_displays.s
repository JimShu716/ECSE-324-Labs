...
HEX_clear_ASM: 				
			...
			MOV R4, #0	 // counter
clear03:  
			CMP R4, #4	 // we only loop 4 times for HEX 0-3
			BGE goto45     // change address to 4-5 if 0-3 already checked
			TST R0, R3	 // AND R0 with R3   //r0??  AND r3??
...
clear:	  
			ADD R5, R4, R1 // get the next HEX location    //BYTE addressable , address+=1  ==???byte
			MOV R6, #0      			
			STRB R6, [R5]	 // clear the location
			LSL R3, R3, #1 // left shift comparison value    //?????hex ??????
			ADD R4, R4, #1 // update counter
			B clear03      // check the next HEX location
goto45:   
			LDR R2, =HEX45 // store address of 4 and 5 to R2
			MOV R4, #0	 // reset counter
clear45:  
			CMP R4, #2	 // check for max 2 times			
			BXGE LR	     
			...
			B clear45
clear2:	  
			ADD R5, R4, R2 // get the next HEX location
			MOV R6, #0
			STRB R6, [R5]	 // clear the location
...
flood:	  
			ADD R5, R4, R1 // get the next location
			MOV R6, #255   // 11111111	
			...
			B flood03      // check the next HEX
end03:    
			LDR R2, =HEX45 // load the address for 4-5
			MOV R4, #0     // initialize the counter
flood45:  
			CMP R4, #2	 // max 2 loops in this case
			BXGE LR
			TST R0, R3     // AND with R3
...
flood2:	  
			ADD R5, R4, R2 // get the exact location
			MOV R6, #255   // 11111111 
			STRB R6, [R5]  // flood
			LSL R3, R3, #1 // left shift the comparison value
			ADD R4, R4, #1 // update the counter
...
write03:  
			ADD R6, R5, R2  // in HEX 0-3, go to the target HEX location
...
write45:  
			ADD R6, R5, R3  // in HEX 0-3, go to the target HEX location
			STRB R1, [R6]	  // overwrite the target location
			LSL R4, R4, #1  // left shift the comparison value
			ADD R5, R5, #1  // update the counter
			B write		  // check the next HEX
